...
Randomly split the data set in two data sets of equal size

```{r split data}
dt = sort(sample(nrow(niris), nrow(niris)*0.5))
train = niris[dt, ]
test = niris[-dt, ]
```
...
miss_lda = mean(ifelse(test[, 1] == new.pred.lda$class, 0, 1))
miss_qda = mean(ifelse(test[, 1] == new.pred.qda$class, 0, 1))
miss_lda
miss_qda
```
